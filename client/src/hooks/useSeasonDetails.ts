...
const useSeasonDetails = (
  ...
  const { data: season = null, isLoading: seasonLoading } = useQuery<SeasonRecord | null>({
    ...
    queryFn: async () => {
      try {
        const { data } = await axios.get<SeasonRecord>(`${API}/admin/seasons/${seasonId}`, {
          headers: authHeaders(),
        });
        return data;
      } catch (err) {
  ...
  const { data: seasonTeams = [], isLoading: seasonTeamsLoading } = useQuery<SeasonTeam[]>({
    ...
    queryFn: async () => {
      try {
        const { data } = await axios.get<SeasonTeam[]>(`${API}/admin/seasons/${seasonId}/teams`, {
          headers: authHeaders(),
        });
        return data;
      } catch (err) {
  ...
  const resetSeasonGroupTeams = async (groupId: string): Promise<boolean> => {
    setBusy(groupId);
    try {
      await axios.delete(`${API}/admin/seasons/${seasonId}/groups/${groupId}/teams`, {
        headers: authHeaders(),
      });
  ...
  const startSeason = async (): Promise<boolean> => {
    setBusy('start-season');
    try {
      await axios.patch(
        `${API}/admin/seasons/${seasonId}/start`,
        {},
        { headers: authHeaders() },
      );
      ...
      await Promise.all([
        ...
        queryClient.invalidateQueries({ queryKey: ['team-seasons'] }),
      ]);
      return true;
    } catch (err) {
      toast.error(apiError(err, 'Failed to start season'));
      return false;
    } finally {
      setBusy(null);
    }
