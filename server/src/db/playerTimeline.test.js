...
describe('ensurePlayerTimelineSchema', () => {
  it('installs temporal affiliations, jersey assignments, projections, and the compatibility view', async () => {
    ...
    const statements = sql.mock.calls.map(([strings]) => strings.join(' ')).join('\n');
    expect(statements).toContain('ADD COLUMN IF NOT EXISTS is_prospect');
    expect(statements).toContain('CREATE TABLE IF NOT EXISTS player_jersey_stints');
    expect(statements).toContain('CREATE TABLE IF NOT EXISTS season_projected_lineup_slots');
    expect(statements).toContain("canonical_player_timelines_v1");
    expect(statements).toContain("canonical_player_timelines_v2_exclusive_boundaries");
    expect(statements).toContain('CREATE OR REPLACE VIEW season_participant_teams');
    ...
    expect(statements).toContain('CREATE OR REPLACE VIEW player_season_rosters');
