  ...
  it("creates roster rows and reports skipped duplicates", async () => {
    sql
      ...
      .mockResolvedValueOnce([
        { id: "career-stint-1", player_id: "player-1", created: true },
      ])
      .mockResolvedValueOnce([])
      .mockResolvedValueOnce([]);
    ...
    expect(
      sql.mock.calls.some((call) =>
        call[0].join(" ").includes("INSERT INTO player_teams"),
      ),
    ).toBe(false);
  });
});
...
describe("POST /api/admin/player-teams", () => {
  it("rejects invalid acquisition_type", async () => {
    const res = await request(app).post("/api/admin/player-teams").send({
      player_id: "player-1",
      team_id: "team-1",
      season_id: "season-1",
...
describe("GET /api/admin/player-teams/history/:playerId", () => {
  it("returns stints with team data nested under team", async () => {
    ...
    const res = await request(app).get(
      "/api/admin/player-teams/history/player-1",
    );
    ...
    const openRosterIndex = queryText.indexOf(
    ...
    const openStintIndex = queryText.indexOf(
  ...
  it("marks stints with team stats as not deletable", async () => {
    sql.mockResolvedValueOnce([
      { ...STINT_ROW, has_player_stats: true, has_goalie_stats: false },
    ]);
...
describe("POST /api/admin/player-teams/history/:playerId/reconcile", () => {
  const mockReconcileReads = (existingStints = [], leagueCode = "NHL") => {
    sql
      .mockResolvedValueOnce([
        { id: RECONCILE_PLAYER_ID, league_player_number: "8478402" },
      ])
      .mockResolvedValueOnce([
        { id: RECONCILE_TEAM_ID, league_code: leagueCode },
      ])
  ...
  it("atomically applies only player_team_stints writes behind an advisory lock", async () => {
    ...
    expect(transactionQueries[0].strings.join(" ")).toContain(
  ...
  it("returns 409 when the under-lock state guard detects a stale preview", async () => {
    mockReconcileReads();
    sql.transaction.mockRejectedValueOnce(
      Object.assign(new Error("state changed"), { code: "22P02" }),
    );
  ...
  it("preserves a manual override while applying other source-owned updates", async () => {
    ...
    sql.transaction.mockImplementationOnce(async (buildQueries) => {
      ...
      return [
        [],
        [],
        [{ id: "career-stint-1", runtime_conflicts: ["acquisition_type"] }],
        [],
      ];
    });
...
describe("PATCH /api/admin/player-teams", () => {
  it("requires an effective date when changing a jersey number", async () => {
    const res = await request(app).patch("/api/admin/player-teams").send({
      player_id: "player-1",
      team_id: "team-1",
      season_id: "season-1",
      jersey_number: 91,
...
describe("POST /api/admin/player-teams/trade", () => {
  ...
  it("keeps acquisition_type null when explicitly provided as unknown", async () => {
    ...
    const careerStintUpsert = sql.mock.calls.find((call) =>
      call[0].join(" ").includes("WITH existing AS"),
    );
    expect(careerStintUpsert?.[0].join(" ")).toContain(
...
describe("DELETE /api/admin/player-teams/:id", () => {
  it("deletes a career stint when the player has no stats for that team", async () => {
    sql
      .mockResolvedValueOnce([
        { id: "career-stint-1", player_id: "player-1", team_id: "team-1" },
      ])
      .mockResolvedValueOnce([
        { has_player_stats: false, has_goalie_stats: false },
      ])
      .mockResolvedValueOnce([]);

    const res = await request(app).delete(
      "/api/admin/player-teams/career-stint-1",
    );
  ...
  it("returns 409 when the player has stats for that team", async () => {
    sql
      ...
      .mockResolvedValueOnce([
        { has_player_stats: true, has_goalie_stats: false },
      ]);
  ...
  it("removes the player-team association", async () => {
    sql
      .mockResolvedValueOnce([])
      .mockResolvedValueOnce([
        { id: "stint-1", player_id: "player-1", team_id: "team-1" },
      ])
      .mockResolvedValueOnce([
        { has_player_stats: false, has_goalie_stats: false },
      ])
  ...
  it("returns 404 when the stint is not found", async () => {
    ...
    const res = await request(app).delete(
      "/api/admin/player-teams/missing-stint",
    );
...
describe("POST /api/admin/player-teams/history/:playerId/jerseys", () => {
  it("records a player-wide jersey change without team context", async () => {
    const assignment = {
      id: "j-1",
      player_id: "player-1",
      jersey_number: 77,
    ...
    sql.mockResolvedValueOnce([assignment]);

    const res = await request(app)
      .post("/api/admin/player-teams/history/player-1/jerseys")
      .send({ jersey_number: 77, effective_date: "2026-06-26" });

    expect(res.status).toBe(201);
    expect(res.body).toEqual({ changed: true, assignment });
    const queryText = sql.mock.calls[0][0].join(" ");
    expect(queryText).toContain("WHERE player_id =");
    expect(queryText).not.toContain("team_id");
  });

  it("validates player-wide jersey changes", async () => {
    const res = await request(app)
      .post("/api/admin/player-teams/history/player-1/jerseys")
      .send({ jersey_number: 177, effective_date: "not-a-date" });
...
describe("GET /api/admin/player-teams/history/:playerId/jerseys", () => {
  it("returns jersey history rows", async () => {
    ...
    const res = await request(app).get(
      "/api/admin/player-teams/history/player-1/jerseys",
    );

    expect(res.status).toBe(200);
    expect(res.body).toHaveLength(1);
    expect(res.body[0].jersey_number).toBe(16);
    expect(sql.mock.calls[0][0].join(" ")).not.toContain("team_id");
  });
});
...
describe("DELETE /api/admin/player-teams/history/jerseys/:id", () => {
  it("deletes a jersey assignment and reconnects the timeline", async () => {
    ...
    const res = await request(app).delete(
      "/api/admin/player-teams/history/jerseys/j-1",
    );
  ...
  it("returns 404 when the jersey history row is missing", async () => {
    ...
    const res = await request(app).delete(
      "/api/admin/player-teams/history/jerseys/missing",
    );
...
describe("DELETE /api/admin/player-teams/history/photos/:id", () => {
  ...
  it("returns 404 when the player photo row is missing", async () => {
    ...
    const res = await request(app).delete(
      "/api/admin/player-teams/history/photos/missing",
    );
